...
func deploy() (err error) {
	...
	if len(sutZipFile) > 0 {
		...
		var basedir string
		if basedir, err = findFileInZip(zipFile, s.Size(), testRunnerFilename); err != nil {
	...
	if len(sutTestRunnerFile) > 0 {
		const rootPath = "/"
		srcFS := os.DirFS(rootPath)
		if _, err = copyFile(srcFS, sutTestRunnerFile, sutWorkdir); err != nil {
			return
		}
	}
...
func init() {
	...
	DockerDeploySutCmd.Flags().StringVar(&sutTestRunnerFile, "testrunner-override", "", "testrunner.textproto file overriding the one in the zip archive")
